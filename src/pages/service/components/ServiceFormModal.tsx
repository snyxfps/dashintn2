        ...
        <div className="space-y-4">
          ...
          {isRCV && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input value={form.agidesk_ticket} onChange={(e) => setForm((f) => ({ ...f, agidesk_ticket: e.target.value }))} />
              </div>

              <div className="space-y-1.5">
                <Label>Tipo de Integração</Label>
                <Input value={form.integration_type} onChange={(e) => setForm((f) => ({ ...f, integration_type: e.target.value }))} />
              </div>
            </div>
          )}
          ...
          {form.status === "NOVO" && (
            <div className="space-y-1.5">
              ...
              <Input type="date" value={form.cadastro_date} onChange={(e) => setForm((f) => ({ ...f, cadastro_date: e.target.value }))} />
            </div>
          )}
          ...
          {form.status === "REUNIAO" && (
            <div className="space-y-1.5">
              ...
              <Input type="datetime-local" value={form.meeting_datetime} onChange={(e) => setForm((f) => ({ ...f, meeting_datetime: e.target.value }))} />
            </div>
          )}
          ...
          {form.status === "DEVOLVIDO" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input type="date" value={form.devolucao_date} onChange={(e) => setForm((f) => ({ ...f, devolucao_date: e.target.value, start_date: e.target.value || f.start_date }))} />
              </div>
              <div className="space-y-1.5">
                <Label>Comercial *</Label>
                <Input placeholder="Responsável comercial" value={form.commercial} onChange={(e) => setForm((f) => ({ ...f, commercial: e.target.value }))} />
              </div>
            </div>
          )}
