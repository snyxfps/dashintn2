...
import {
  Users, TrendingUp, CheckCircle, XCircle, RotateCcw, Activity,
  Trophy, RefreshCw
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  // Top service
  const topService: (Service & { count: number }) | null = services.reduce<(Service & { count: number }) | null>((top, s) => {
    const count = filtered.filter(r => r.service_id === s.id).length;
    return !top || count > top.count ? { ...s, count } : top;
  }, null);

  // Status bar chart data
  const statusChartData = (['NOVO', 'REUNIAO', 'ANDAMENTO', 'FINALIZADO', 'CANCELADO', 'DEVOLVIDO'] as RecordStatus[]).map(s => ({
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-4">
                ...
                {/* Top Serviço */}
                {topService?.name && (
                  <div className="corp-card p-4" style={{ background: 'linear-gradient(135deg, hsl(222 75% 28%) 0%, hsl(213 90% 40%) 100%)' }}>
